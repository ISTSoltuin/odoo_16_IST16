...
class SinespClient(models.Model):
    """
    ...
    """
    _name = 'sinesp.client'
    _description = 'SINESP Cidadão Client'
    
    body_file = fields.Binary(
        string='Request Body',
        readonly=True,
        required=True,
        filters=None,
        default=lambda self: self.get_xml_body()
    )
    
    def get_xml_body(self):
        body_xml = self.env.ref('wedrive_beta.sinesp_request_body')
        return body_xml
    ...
    def _parse(self, response):
        ...
        try:
            ...
            raise ValidationError('Could not parse request response.')
