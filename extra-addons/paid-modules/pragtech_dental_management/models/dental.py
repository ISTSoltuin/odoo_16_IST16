...
class Partner(models.Model):
    ...
    treatment_id = fields.Many2many('product.product', 'partner_product_relation', 'partner_id',
...
class ProductProduct(models.Model):
    ...
    insurance_company_id = fields.Many2many('res.partner', 'product_partner_relation', 'product_id', 'partner_id', string='Insurance Company')
...
class MedicalPatient(models.Model):
    ...
    mobile = fields.Char('Mobile', related='partner_id.mobile', readonly=False, store=True)
    ...
    #       print("\n\n\n\n============return_warnings",self)
    #       dict =  {
    #                 "name": 'test1',
    #                 "description": self.medical_history,
    #                 "css_class": 'info',
    #             }
    #       return dict
...
class MedicalAppointment(models.Model):
    ...
    app_hour = fields.Many2one('hour.select', 'Hour')
    app_minute = fields.Many2one('minute.select', 'Minute')

    @api.onchange('app_hour', 'app_minute', 'appointment_sdate')
    def onchange_app_hour(self):
        minutes = self.app_minute.number or 0
        hours = (self.app_hour.number * 60) or 0
    ...
    def write(self, vals):
        for appointmnet in self.search([('id', '!=', self.id)]):
            ...
            if appointmnet.doctor.id == vals.get('doctor'):
                ...
                reservation_start_date = datetime.strptime(str(appoint_sdate), '%Y-%m-%d %H:%M:%S')
                reservation_end_date = datetime.strptime(str(appoint_edate), '%Y-%m-%d %H:%M:%S')
                if (reservation_end_date and history_end_date) and (
                    ...
                    raise ValidationError(
                        _('Doctor  %s is booked in this reservation period !') % (appointmnet.doctor.name))
