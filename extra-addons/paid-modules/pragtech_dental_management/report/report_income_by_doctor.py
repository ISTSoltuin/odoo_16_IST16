...
class ReportIncomeByDoctor(models.AbstractModel):
    ...
    def fetch_record(self, start_date, end_date):
        invoice_ids=self.env['account.move'].search([('invoice_date','>=',start_date),('invoice_date','<=',end_date),('dentist','!=',False),('state','=','posted'),('payment_state','in',['paid','partial']),('move_type','=','out_invoice')])
        res=[]
        for each_record in invoice_ids:
            if not res:
                if each_record.payment_state == 'partial':
                    res.append(
                        {'dentist_id': each_record.dentist.id,
                         'dentist_name': each_record.dentist.name,
                         'customer_count': 1,
