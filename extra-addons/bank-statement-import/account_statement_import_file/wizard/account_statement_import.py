...
class AccountStatementImport(models.TransientModel):
    ...
    def import_file_button(self):
        ...
        try:
            ...
            account_name = result[0][1]
            account_bank_statment_id = self.env['account.bank.statement'].search([
                ('name','=', account_name)
            ], limit=1, order='id desc')
            journal_id = self.env['account.journal'].search([
                ('id','=',self.env.context.get('journal_id'))
            ], limit=1, order='id desc')
            self.create_account_bank_statment_lines(result, account_bank_statment_id)
            domain = [("journal_id", "=", journal_id.id)]
            # TODO: 
            #   1. Refatorar rotina para esse cenário de importar e visualizar entradas.
            #   2. Ajustar cenário para importação com reconciliação simultanea.
            ...
            return {
                ...
                "name": f"Extratos Bancários {journal_id.display_name}",
                "domain": domain
            }
    ...
    def _prepare_create_attachment(self, result):
        ...
        try:
            account_bank_statment_code = result[0][1]
            statement_id = self.env['account.bank.statement'].search([
                ('name','=',account_bank_statment_code)
            ], limit=1, order='id desc')
    ...
    def create_account_bank_statment_lines(self, result, account_bank_statment_id):
        try:
            AccountBankStatementLineObj = self.env['account.bank.statement.line']
            account_bank_statement_lines = result
            journal_id = self.env['account.journal'].search([
                ('id','=',self.env.context.get('journal_id'))
            ], limit=1, order='id desc')
            for account_bank_statement_line in account_bank_statement_lines[0][2][0]['transactions']:
                
                vals = {
                    'date': account_bank_statement_line['date'].date(),
                    'payment_ref': account_bank_statement_line['payment_ref'],
                    'amount': account_bank_statement_line['amount'],
                    'journal_id': journal_id.id,
                    'company_id': self.env.company.id if self.env.company else self.env.user.company_id.id,
                    'account_number': account_bank_statement_lines[0][1],
                    'unique_import_id': account_bank_statement_line['unique_import_id']
                }
                
                account_bank_statement_line_id = AccountBankStatementLineObj.search([
                    ('unique_import_id','=',account_bank_statement_line['unique_import_id'])
                ])

                if not account_bank_statement_line_id:
                    AccountBankStatementLineObj.create(vals)
                else:
                    account_bank_statement_line_id.write(vals)
            
            new_entry_ids = AccountBankStatementLineObj.search([
                ('journal_id','=',self.env.context.get('journal_id'))
            ])

            for new_entry_id in new_entry_ids:
                if new_entry_id.id not in account_bank_statment_id.line_ids.ids:
                    account_bank_statment_id.write({
                        'line_ids': [(4, new_entry_id.id)]
                    })
            previous_balance_end_real = account_bank_statment_id.balance_end_real
            balance_new_entries = sum(new_entry_ids.mapped('amount'))
            account_bank_statment_id.balance_end_real = previous_balance_end_real + balance_new_entries
        except Exception as e:
            raise UserError(e)